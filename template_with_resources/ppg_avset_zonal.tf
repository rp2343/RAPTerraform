...
resource "azurerm_proximity_placement_group" "ppg" {
  count               = length(var.subnet_prefix)
  name                = "ppg${count.index +1}"
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
  tags                = var.tags
}

# Create network interface for PPG Anchor VM
resource "azurerm_network_interface" "ppgnic" {
  count               = length(var.subnet_prefix)
  name                = "anchornic${count.index + 1}"
  location            = var.location
  resource_group_name = azurerm_resource_group.rg.name
  tags                          = var.tags
...
# Create PPG anchor Linux virtual machines 
resource "azurerm_virtual_machine" "ppgvm" {
  count               = length(var.subnet_prefix)
  name                = "anchorvm${count.index + 1}"
  location            = var.location
  resource_group_name = azurerm_resource_group.rg.name
  network_interface_ids = [azurerm_network_interface.ppgnic[count.index].id]
  vm_size               = "Standard_D4s_v3"
  # zones                 = [element(split(",", var.av_zone), count.index)]
  zones 				= [element(var.av_zones, count.index)]
  proximity_placement_group_id = element(azurerm_proximity_placement_group.ppg.*.id, count.index)
  # availability_set_id   = element(azurerm_availability_set.avset.*.id, count.index)
  tags  = var.tags

  storage_os_disk {
    name              = "anchorOsDisk${count.index + 1}"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "anchorVM${count.index + 1}"
...
# Create Availabiltity Sets
resource "azurerm_availability_set" "avset" {
  count               = length(var.subnet_prefix)
  name                = "Avset${count.index +1}"
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
  proximity_placement_group_id = element(azurerm_proximity_placement_group.ppg.*.id, count.index)
  tags                = var.tags
