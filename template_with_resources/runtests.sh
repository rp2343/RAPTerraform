...
ssh $vm1 qperf &
echo "Starting iperf test with Strongswan Disabled"
for client in `cat iplist`; do
ssh -o StrictHostKeyChecking=no $client "mkdir -p no_encryption; cd no_encryption; echo -e Executing IPERF Throughput test '\\n\\n' >> iperf_\$(hostname)_$(date +%Y%m%d).txt ;for block in 4k 8k 16k 64k 128k 256k; do echo Executing iperf for \$block >> iperf_\$(hostname)_$(date +%Y%m%d).txt; iperf3 -c $vm1 --port 5201 -l \$block -w \$block -P 35 |grep -i SUM |tail -n2 >> iperf_\$(hostname)_$(date +%Y%m%d).txt ; sleep 5s; echo -e '\n' >> iperf_\$(hostname)_$(date +%Y%m%d).txt; done"
ssh -o StrictHostKeyChecking=no $client "cd no_encryption; echo -e Executing QPERF Latency test '\\n\\n' >> qperf_\$(hostname)_$(date +%Y%m%d).txt ;for block in 4k 8k 16k 64k 128k 256k; do echo Executing qperf for \$block >> qperf_\$(hostname)_$(date +%Y%m%d).txt; qperf --ip_port 19766 --msg_size "$i"k -t 60 $vm1 tcp_bw tcp_lat >> qperf_\$(hostname)_$(date +%Y%m%d).txt ; sleep 5s; echo -e '\n' >> iperf_\$(hostname)_$(date +%Y%m%d).txt; done"
...
ssh -o StrictHostKeyChecking=no $client "cd no_encryption; echo -e Executing TCPPING Latency test '\\n\\n' >> tcpping_\$(hostname)_$(date +%Y%m%d).txt ; tcpping -w 1 -x 60 $vm1 80 >> tcpping_\$(hostname)_$(date +%Y%m%d).txt; sleep 5s, echo -e '\n' >> tcpping_\$(hostname)_$(date +%Y%m%d).txt; done"
ssh -o StrictHostKeyChecking=no $client "mv no_encryption no_encryption_$(date +%Y%m%d_%H%M%S)"
done


echo "Executing iperf with strongswan Enabled"
for client in `cat iplist`; do
